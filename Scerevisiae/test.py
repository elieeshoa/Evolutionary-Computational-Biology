...
# print("FBA summary", Scerevisiae_model.summary)
...
with open("scerevisiae-aerobic-output.txt", "w") as f:
    ...
    print("", file=f)
    for x in aerobic:
        print(f"{x} : " + fba_solution_biomass.fluxes.get(x).__str__(), file=f)
    print("", file=f)
    # print("Time elapsed to solve the FBA problem: " + str(end - start), file=f)
    print("Objective value: " + str(fba_solution_biomass.objective_value), file=f)
    print("", file=f)
        ...
        if fba_solution_biomass.fluxes.get(key=y):
            if fba_solution_biomass.fluxes.get(key=y) > 0:
                print('exchange flux metabolites secreted', x.name, \
                      fba_solution_biomass.fluxes.get(key=y), file=f)
