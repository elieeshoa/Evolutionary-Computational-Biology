...
class metabolite(object):
    ...
    def __init__(self, id, compartment = None, name = None, Kegg_id = None, ModelSeed_id = None, formula = None, model = None, reactions = [], reactant_reactions = [], product_reactions = [], concentration = None, deltaGf_range = []): 
        ...
        # The model in which this metabolite participates
        self.model = model
        ...
        # reactant or product
        if reactions == [] and (len(reactant_reactions) >= 1 or len(product_reactions) >= 1):
        ...
        # List of reaction objects in which this metabolite participates as a
        # reactant 
        if reactant_reactions == [] and len(self.reactions) >= 1:
            self.reactant_reactions = [r for r in self.reactions if r.stoichiometry[self] < 0] 
        else:
        ...
        if product_reactions == [] and len(self.reactions) >= 1:
            self.product_reactions = [r for r in self.reactions if r.stoichiometry[self] > 0]
        else:
    ...
    def biomass_yield_calc(self,fba_model = None):
        ...
        if len(exch_rxn) == 0:
            ...
            LBorig = exch_rxn.flux_bounds[0]
  
            # Perform FBA for 10 mole uptake of this metabolite 
            exch_rxn.flux_bounds[0] = -10      

            if fba_model == None:
                print 'WARNING! No fba_model was provided. The current model.fba_model is used to compute the biomass yield'
                self.model.fba(create_model = False,screen_output = 'off')
            ...
            if fba_solution['exit_flag'] == 'globallyOptimal':
                self.biomass_yield = fba_solution['objective_value']
            else:
