...
class model(object):
    ...
    def __init__(self, id, type, organism, reactions = None, compounds = None, genes = None, compartments = None, name = None, biomass_reaction = None, atpm_reaction = None, simulation_condition = None, fba_model = None, wildType_max_biomass = None, notes = None):
    ...
    def get_reactions(self,reactions_ref,search_by_clean_ref = False):
        ...
        for rxn_ref in reactions_ref.keys(): 
            if not search_by_clean_ref:
                rxn_object = []
                if reactions_ref[rxn_ref].lower() == 'id':
                    rxn_object = [r for r in self.reactions if remove_non_alphanumeric(r.id).lower() == remove_non_alphanumeric(rxn_ref).lower()]
                elif reactions_ref[rxn_ref].lower() == 'name':
                    rxn_object = [r for r in self.reactions if isinstance(r.name,str) and remove_non_alphanumeric(r.name).lower() == remove_non_alphanumeric(rxn_ref).lower() or isinstance(r.name,list) and remove_non_alphanumeric(rxn_ref).lower() in [remove_non_alphanumeric(n) for n in r.name]]
                else:
            ...
            # Search by actual (not clean) names
                ...
                if reactions_ref[rxn_ref].lower() == 'id':
                    ...
                    rxn_object = [r for r in self.reactions if isinstance(r.name,str) and r.name.lower() == rxn_ref.lower() or isinstance(r.name,list) and r.name.lower() in [n.lower() for n in r.name]]
                else:
                    raise userError("**Error! Invalid reaction reference '" + reactions_ref[rxn] + "'")
        ...
        for cmp_ref in compounds_ref.keys(): 
            ...
            if search_by_clean_ref:
                if compounds_ref[cmp_ref].lower() == 'id':
                    cmp_object = [m for m in self.compounds if remove_non_alphanumeric(m.id).lower() == remove_non_alphanumeric(cmp_ref).lower()]
                    ...
                    raise userError("Invalid compound reference (" + cmp_ref + ")")

            # Search with actual (not clean) reference
            else: 
                if compounds_ref[cmp_ref].lower() == 'id':
                    cmp_object = [m for m in self.compounds if m.id.lower() == cmp_ref.lower()]
                elif compounds_ref[cmp_ref].lower() == 'name':
    ...
    def add_reactions(self,new_reactions):
        ...
        cmp_notInMoDel = [c for r in new_reacitons for c in r.compounds if c not in self.reactions]
        if len(cmp_notInMoDel) > 0:
            raise ValueError('The following compounds not in the model ' + model.id + ': ' + str([c.id for c in cmp_notInMoDel]))
    ...
    def validate(self):
        ...
        if len(set(cmp_ids)) < len(cmp_ids):
            ...
            print '\tThe following compound ids are repeated: ',repeated_cmp_ids 
        ...
        if fixed_cmp > 0:
            print '\t',fixed_cmp,' compounds not participated in any reactions in the model were fixed'
        ...
        if fixed_rxn > 0:
            print '   ',fixed_rxn," reactions with empty field 'compounds' were fixed"
        ...
        for rxn in self.reactions:
            ...
            rxn_cmps = [c for c in self.compounds if rxn in c.reactions or rxn in c.reactant_reactionss or rxn in c.product_reactions]
            if set(rxn_cmps) != set(rxn.compounds):
                ...
                for c in [c for c in rxn_cmps if rxn in c.reactant_reactions and c not in rxn.reactants]:
                    ...
                    if c not in rxn.stoichiometry.keys():
                        raise userError('Uknown stoichiometric coefficient for compound ' + c.id + ' in rxn ' + rxn.id)
                    elif rxn.stoichiometry[c] > 0:
