...
class model(object):
    ...
    def validate(self, reassign_props = True):
        ...
        cpds_rxns_dict = [(c.id,len(c.reactions)) for c in self.compounds]
        ...
        if 0 in list(set([s for r in self.reactions for s in r.stoichiometry.values()])):
            ...
            for cpd in [c for c in self.compounds if len([r for r in c.reactions if r.stoichiometry[c] == 0]) > 0]:
                ...
                for rxn in [r for r in cpd.reactions if r.stoichiometry[cpd] == 0]:
                    # Remove this reaciton from cpd.reactions, cpd.reactant_reactions or cpd.product_reactions
        ...
        # for each compound in the model
        # Checks and fixes any compound that is referred to in a reaction.stoichiometry,
        # reaction.compounds, or reaction.reactants but is not in the list of compound.reactions,
        # compound.reactant_reactions or compound.product_reactions
        cpd_counter = 0
        for cpd in self.compounds:
            # Reactions
            for rxn in [r for r in self.reactions if cpd in r.compounds and r not in cpd.reactions]:
                cpd.reactions.append(rxn)
                cpd_counter += 1

            for rxn in [r for r in cpd.reactions if (cpd not in r.stoichiometry.keys() or r.stoichiometry[cpd] == 0)]:
                del cpd.reactions[cpd.reactions.index(rxn)]
                cpd_counter += 1

            # Reactants
            for rxn in [r for r in self.reactions if cpd in r.reactants and r not in cpd.reactant_reactions]:
                cpd.reactant_reactions.append(rxn)
                cpd_counter += 1

            for rxn in [r for r in cpd.reactant_reactions if (cpd not in r.stoichiometry.keys() or r.stoichiometry[cpd] >= 0)]:
                del cpd.reactions[cpd.reactions.index(rxn)]
                cpd_counter += 1

            # Products
            for rxn in [r for r in self.reactions if cpd in r.products and r not in cpd.product_reactions]:
                cpd.product_reactions.append(rxn)
                cpd_counter += 1

            for rxn in [r for r in cpd.product_reactions if (cpd not in r.stoichiometry.keys() or r.stoichiometry[cpd] <= 0)]:
                del cpd.reactions[cpd.reactions.index(rxn)]
                cpd_counter += 1

            if set([r for r in self.reactions if cpd in r.compounds]) != set([r for r in self.reactions if cpd in r.stoichiometry.keys()]):
                raise userError('The set of reactions in the model in which compound ' + cpd.id + ' appears in their "compounds" field does not match the set of reactions in which this compounds appears with a nonzero stoichiometry')
            if set([r for r in self.reactions if cpd in r.reactants]) != set([r for r in self.reactions if cpd  in r.stoichiometry.keys() and r.stoichiometry[cpd] < 0]):
                raise userError('The set of reactions in the model in which compound ' + cpd.id + ' appears in their "reactants" field does not match the set of reactions in which this compounds appears with a negative stoichiometry')
            if set([r for r in self.reactions if cpd in r.products]) != set([r for r in self.reactions if cpd in r.stoichiometry.keys() and r.stoichiometry[cpd] > 0]):
                raise userError('The set of product_reactions in the model in which compound ' + cpd.id + ' appears in their "products" field (i.e., ' + str(set([r.id for r in self.reactions if cpd in r.products])) + ') does not match the set of reactions in which this compounds appears with a negative stoichiometry (i.e., ' + str(set([r.id for r in self.reactions if cpd in r.stoichiometry.keys() and r.stoichiometry[cpd] > 0])) + ')')

        if cpd_counter > 0:
            errors_in_model = True
            if self.stdout_msgs:
                print '\t{} compounds with issues in their "reactions", "reactant_reactions" and "product_reactions" were fixed ...'.format(cpd_counter)

        # Removes any duplicates in fields reactions, reactant_reactions and product_reactions
        # for each compound in the model
        self.compounds = list(set(self.compounds))
        for cpd in self.compounds:
        ...
        missing_rxns = list(set([r for c in self.compounds for r in c.reactions + c.reactant_reactions + c.product_reactions if r not in self.reactions])) 
        ...
        for rxn in self.reactions:
            # Compounds
            for cpd in [c for c in rxn.compounds if (c not in rxn.stoichiometry.keys() or rxn.stoichiometry[c] == 0)]:
                del rxn.compounds[rxn.compounds.index[cpd]]
                rxn_counter += 1
            # Reactants
            for cpd in [c for c in  rxn.reactants if (c not in rxn.stoichiometry.keys() or rxn.stoichiometry[c] == 0 or rxn.stoichiometry[c] > 0)]:
                del rxn.reactants[rxn.reactants.index(cpd)] 
                rxn_counter += 1
            # Products
            for cpd in [c for c in r.products if (c not in r.stoichiometry.keys() or r.stoichiometry[c] < 0)]:
                del rxn.products[rxn.products.index(cpd)] 
                rxn_counter += 1

            # Reactions
            for cpd in [c for c in self.compounds if rxn in c.reactions and c not in rxn.compounds]:
                rxn.compounds.append(cpd)
                rxn_counter += 1
            # reactant_reactions
            for cpd in [c for c in self.compounds if rxn in c.reactant_reactions and c not in rxn.reactants]:
                rxn.reactants.append(cpd)
                rxn_counter += 1
            # product_reactions
            for cpd in [c for c in self.compounds if rxn in c.product_reactions and c not in rxn.products]:
                rxn.products.append(cpd)
                rxn_counter += 1
            ...
            rxn_cpds = [c for c in self.compounds if rxn in c.reactions or rxn in c.reactant_reactions or rxn in c.product_reactions]
            if set(rxn_cpds) != set(rxn.compounds):
                rxn_counter += 1
                for c in [c for c in rxn_cpds if c not in rxn.compounds]:
                    rxn.compounds.append(c)

                for c in [c for c in rxn_cpds if rxn in c.reactant_reactions and c not in rxn.reactants]:
                ...
                for c in [c for c in rxn_cpds if rxn in c.product_reactions and c not in rxn.products]:
