...
class model(object):
    ...
    --------
            ...
            set_model_id: Assigns a model id to the model and its relatd objects (compounds, rxns, genes, etc.)
    ...
    """
    def __init__(self, id, type = 'metabolic', organism = None, compounds = (), reactions = (), genes = (), compartments = (), name = None, biomass_reaction = None, atpm_reaction = None, notes = None, validate = True, stdout_msgs = False, warnings = True):
        ...
        # Assign model id
        self.set_model_id(model_id = self.id)
    ...
    def __setattr__(self,attr_name,attr_value):
       ...
       -------
        ...
        if attr_name == 'id':
            self.set_model_id(model_id = attr_value)
        elif attr_name == 'compounds':
    ...
    def set_model_id(self, model_id):
        ...
        if not isinstance(model_id,str):
            raise TypeError("Invlaid id for model " + str(model_id) + "! id must be a string. A " + str(type(model_id)) + " type object was entered instead")

        self.__dict__['id'] = model_id

        # Anytime the id of the model changes, the id of compounds, reactions genes, etc should 
        # be updated as well
        #-- organism --
        if hasattr(self,'organism') and self.organism != None:
            self.organism.model_id = self.id

        #-- Compartments --
        if hasattr(self, 'compartments') and self.compartments != None:
            for cpt in self.compartments:
                cpt.model_id = self.id
        ...
        if hasattr(self, 'reactions'):
            for rxn in self.reactions:
                rxn.model_id = self.id 

        #-- Genes --
        if hasattr(self, 'genes'):
            for gene in self.genes:
                gene.model_id = self.id
        ...
        for rxn_ref in reactions_ref.keys(): 
            ...
            if search_by_clean_ref:
                ...
                if reactions_ref[rxn_ref].lower() == 'id':
                    ...
                    rxn_object = [r for r in self.reactions if remove_non_alphanumeric(rxn_ref).lower() in [remove_non_alphanumeric(r.name).lower()] + [remove_non_alphanumeric(n).lower() for n in r.name_aliases]]
                elif reactions_ref[rxn_ref].lower() == 'modelseed_id':
                    rxn_object = [r for r in self.reactions if rxn_ref.lower() in [sid.lower() for sid in r.ModelSEED_id]]
                elif reactions_ref[rxn_ref].lower() == 'kegg_id':
                    rxn_object = [r for r in self.reactions if rxn_ref.lower() in [kid.lower() for kid in r.KEGG_id]]
                elif reactions_ref[rxn_ref].lower() == 'name_compart':
                    rxn_object = [r for r in self.reactions if remove_non_alphanumeric(rxn_ref).lower() in [remove_non_alphanumeric(r.name).lower() + '_' + r.compartments[0].id] + [remove_non_alphanumeric(n).lower() + '_' + r.compartments[0].id for n in r.name_aliases]]
                elif reactions_ref[rxn_ref].lower() == 'modelseed_id_compart':
                    rxn_object = [r for r in self.reactions if len(r.compartments) == 1 and remove_non_alphanumeric(rxn_ref).lower() in [remove_non_alphanumeric(sid + '_' + r.compartments[o].id).lower() for sid in r.ModelSEED_id]]
                elif reactions_ref[rxn_ref].lower() == 'kegg_id_compart':
                    rxn_object = [r for r in self.reactions if len(r.compartments) == 1 and remove_non_alphanumeric(rxn_ref).lower() in [remove_non_alphanumeric(kid + '_' + r.compartments[0].id).lower() for kid in r.KEGG_id]]
            ...
            # Search by actual (not clean) names
                ...
                if reactions_ref[rxn_ref].lower() == 'id':
                    ...
                    rxn_object = [r for r in self.reactions if rxn_ref in [r.name] + [n for n in r.name_aliases]]
                elif reactions_ref[rxn_ref].lower() == 'modelseed_id':
                    rxn_object = [r for r in self.reactions if rxn_ref in [sid for sid in r.ModelSEED_id]]
                elif reactions_ref[rxn_ref].lower() == 'kegg_id':
                    rxn_object = [r for r in self.reactions if rxn_ref in [kid for kid in r.KEGG_id]]
                elif reactions_ref[rxn_ref].lower() == 'name_compart':
                    rxn_object = [r for r in self.reactions if rxn_ref in [r.name + '_' + r.compartments[0].id] + [n + '_' + r.compartments[0].id for n in r.name_aliases]]
                elif reactions_ref[rxn_ref].lower() == 'modelseed_id_compart':
                    rxn_object = [r for r in self.reactions if len(r.compartments) == 1 and rxn_ref in [sid + '_' + r.compartments[o].id for sid in r.ModelSEED_id]]
                elif reactions_ref[rxn_ref].lower() == 'kegg_id_compart':
                    rxn_object = [r for r in self.reactions if len(r.compartments) == 1 and rxn_ref in [kid + '_' + r.compartments[0].id for kid in r.KEGG_id]]
    ...
    def get_compounds(self,compounds_ref, search_by_clean_ref = True):
        ...
        for cpd_ref in compounds_ref.keys(): 
            cpd_object = []
            # Search by clean reference
            if search_by_clean_ref:
                if compounds_ref[cpd_ref].lower() == 'id':
                    try:
                        cpd_object = [self.compounds_by_clean_id[remove_non_alphanumeric(cpd_ref).lower()]]
                    except:
                        cpd_object = []
                elif compounds_ref[cpd_ref].lower() == 'name':
                    cpd_object = [m for m in self.compounds if remove_non_alphanumeric(cpd_ref).lower() in [remove_non_alphanumeric(m.name).lower()] + [remove_non_alphanumeric(n) for n in m.name_aliases]]
                elif compounds_ref[cpd_ref].lower() == 'name_compart':
                    cpd_object = [m for m in self.compounds if remove_non_alphanumeric(cpd_ref).lower() in [remove_non_alphanumeric(m.name + '_' + m.compartment.id).lower()] + [remove_non_alphanumeric(n + '_' + m.compartment.id).lower() for n in m.name_aliases]]
                elif compounds_ref[cpd_ref].lower() == 'kegg_id':
                    cpd_object = [m for m in self.compounds if cpd_ref.lower() in [kid.lower() for kid in m.KEGG_id]]
                elif compounds_ref[cpd_ref].lower() == 'kegg_id_compart':
                    cpd_object = [m for m in self.compounds if remove_non_alphanumeric(cpd_ref).lower() in [remove_non_alphanumeric(kid + '_' + m.compartment.id).lower() for kid in m.KEGG_id]]
                elif compounds_ref[cpd_ref].lower() == 'modelseed_id':
                    cpd_object = [m for m in self.compounds if cpd_ref.lower() in [sid.lower() for sid in m.ModelSEED_id]]
                elif compounds_ref[cpd_ref].lower() == 'modelseed_id_compart':
                    cpd_object = [m for m in self.compounds if remove_non_alphanumeric(cpd_ref).lower() in [remove_non_alphanumeric(sid + '_' + m.compartment.id).lower() for sid in m.ModelSEED_id]]
                elif compounds_ref[cpd_ref].lower() == 'formula':
                    cpd_object = [m for m in self.compounds if m.formula != None and remove_non_alphanumeric(m.formula).lower() == remove_non_alphanumeric(cpd_ref).lower()]
                elif compounds_ref[cpd_ref].lower() == 'formula_compart':
                    cpd_object = [m for m in self.compounds if m.formula != None and remove_non_alphanumeric(m.formula + '_' + m.compartment.id).lower() == remove_non_alphanumeric(cpd_ref).lower()]
                else:
                    raise ValueError("Invalid compound reference (" + cpd_ref + ")")

            # Search with actual (not clean) reference
            else: 
                if compounds_ref[cpd_ref].lower() == 'id':
                    try:
                        cpd_object = [self.compounds_by_id[cpd_ref]]
                    except:
                        cpd_object = [] 
                elif compounds_ref[cpd_ref].lower() == 'name':
                    cpd_object = [m for m in self.compounds if cpd_ref in [m.name] + [n for n in m.name_aliases]]
                elif compounds_ref[cpd_ref].lower() == 'name_compart':
                    cpd_object = [m for m in self.compounds if cpd_ref in [m.name + '_' + m.compartment.id] + [n + '_' + m.compartment.id for n in m.name_aliases]]
                elif compounds_ref[cpd_ref].lower() == 'kegg_id':
                    cpd_object = [m for m in self.compounds if cpd_ref in [kid for kid in m.KEGG_id]]
                elif compounds_ref[cpd_ref].lower() == 'kegg_id_compart':
                    cpd_object = [m for m in self.compounds if cpd_ref in [kid + '_' + m.compartment.id for kid in m.KEGG_id]]
                elif compounds_ref[cpd_ref].lower() == 'modelseed_id':
                    cpd_object = [m for m in self.compounds if cpd_ref in [sid for sid in m.ModelSEED_id]]
                elif compounds_ref[cpd_ref].lower() == 'modelseed_id_compart':
                    cpd_object = [m for m in self.compounds if cpd_ref in [sid + '_' + m.compartment.id for sid in m.ModelSEED_id]]
                elif compounds_ref[cpd_ref].lower() == 'formula':
                    cpd_object = [m for m in self.compounds if m.formula != None and m.formula.lower() == cpd_ref.lower()]
                elif compounds_ref[cpd_ref].lower() == 'formula_compart':
                    cpd_object = [m for m in self.compounds if m.formula != None and m.formula.lower() + '_' + m.compartment.id.lower() == cpd_ref.lower()]
                else:
                    raise ValueError("Invalid compound reference (" + cpd_ref + ")")

    
            if len(cpd_object) == 1:
                compound_objects.append((cpd_ref,cpd_object[0])) 
            elif len(cpd_object) > 1:
                if self.warnings:
                    print 'WARNING! More than one compound object found for ', cpd_ref,': ', [m.id for m in cpd_object]
                compound_objects.append((cpd_ref,cpd_object[0])) 
            elif len(cpd_object) == 0:
                if self.warnings:
                    print 'WARNING! No compound object found for ', cpd_ref,': ', [m.id for m in cpd_object]
                compound_objects.append((cpd_ref,None)) 
    ...
    def del_compounds(self,compounds_list):
        ...
        cpds_in_rxns = set([r.compounds for r in self.reactions])
        del_cpds_in_rxns = list(set(compounds_list).intersection(cpds_in_rxns)) 
        if len(del_cpds_in_rxns) > 0:
            if self.warnings:
                print '**WARNING! The following compounds participate in at least one reaction and cannot be removed. Remove the reactions first and then remove the compounds {}'.format([cpd.id for cpd in del_cpds_in_rxns])

        model_cpds = list(self.compounds)
        for cpd in [c for c in compounds_list if c not in cpds_in_rxns]:
    ...
    def del_reactions(self,reactions_list):
        ...
        cpds_only_in_rxnsList = list(set([c for r in reactions_list for c in r.compounds if len(c.reactions) == 1]))

        model_rxns = list(self.reactions)
        for rxn in reactions_list:
            # Remove it from the list of reactions
            del model_rxns[model_rxns.index(rxn)] 
    ...
    def export(self, output_format = 'sbml', output_filename = ''):
        ...
        if output_format.lower() == 'sbml':
            import cobra
            cobra_model = self.convert_to_cobra()
            cobra.io.write_sbml_model(cobra_model,output_filename + '.xml',use_fbc_package=True)       
            if self.stdout_msgs:
        ...
        #--------- pydict -----------
            ...
            #-- Save model to file --
            with open(output_filename + '.py','w') as f:
                f.write('model = {\n')
    ...
    def fba(self,optimization_solver = default_optim_solver, build_new_optModel = True, maximize = True, reset_fluxes = True, store_opt_fluxes = True, store_all_rxn_fluxes = False, flux_key = None, run = True, assign_wildType_max_biomass = True, simulation_conditions = '', stdout_msgs = True, warnings = True):
        """
        ...
        if build_new_optModel:
            self.fba_model = fba(model = self,optimization_solver = optimization_solver, build_new_optModel = True, flux_key = flux_key, store_opt_fluxes = store_opt_fluxes, simulation_conditions = simulation_conditions, maximize = maximize, warnings = warnings, stdout_msgs = stdout_msgs)
        else:
